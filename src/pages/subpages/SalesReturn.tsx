...
export default function SalesReturn({ onBack, shop }: any) {
    ...
    const fetchSales = async () => {
        try {
            setLoading(true);
            const res = await databases.listDocuments(DB_ID, SALES_COLLECTION, [
    ...
    return (
        <div className="h-screen bg-slate-50 flex flex-col">
            <PageHeader title={t.salesReturn || "Sales Return"} onBack={onBack} />
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {loading ? (
                    ...
                    <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-200">
                        ...
                        <p className="text-slate-500 font-medium">No sales found to return</p>
                    </div>
                ) : filteredSales.map(sale => (
                    <div key={sale.$id} className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-all active:scale-[0.98] cursor-pointer group">
                        <div className="flex justify-between items-start mb-3">
                            <div>
                                <div className="flex items-center text-slate-400 text-[10px] font-bold uppercase tracking-wider mb-1">
                                    <Hash className="h-3 w-3 mr-1" /> {sale.invoice_no}
                                </div>
                                <h3 className="font-bold text-slate-800 flex items-center">
                                    <User className="h-4 w-4 mr-2 text-slate-400" /> {sale.customer_name}
                                </h3>
                            </div>
                            <div className="bg-orange-50 text-orange-600 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider">
                                Returnable
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 py-3 border-y border-slate-50">
                            <div className="flex items-center">
                                ...
                                <span className="text-xs font-medium text-slate-600">{new Date(sale.date).toLocaleDateString()}</span>
                            </div>
                            <div className="flex items-center justify-end">
                                <DollarSign className="h-4 w-4 text-emerald-500 mr-1" />
                                <span className="text-sm font-black text-slate-800">{formatCurrency(sale.total_amount)}</span>
                            </div>
                        </div>
                        ...
                        <div className="mt-3 flex justify-between items-center">
                            ...
                            <button className={`p-2 rounded-xl ${themeClasses.lightBg} ${themeClasses.primaryText} hover:bg-opacity-80 transition-all`}>
            ...
            <div className="p-4 bg-white border-t border-slate-100 text-center">
                <p className="text-[10px] text-slate-400 font-medium uppercase tracking-widest">Select a sale to initiate a return process</p>
            </div>
        </div>
    );
}
